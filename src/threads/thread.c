...
//   printf("NOW: %s ", thread_current()->name);
//   char *name;
//   if (list_empty(&ready_list)) name = idle_thread->name;
//   else  name = list_entry (list_front(&ready_list), struct thread, elem)->name;
//   printf(" NEXT: %s\n", name);
...
static void update_priority_mlfqs(struct thread *, void *);
static void update_recent_cpu_mlfqs(struct thread *, void *);
static void update_load_avg_mlfqs(void);
               ...
               thread_func *function, void *aux) 
{
  ...
  if (thread_mlfqs) {
    ...
    t->recent_cpu = (thread_current() == initial_thread) ? 0 : thread_current()->recent_cpu;
  }
...
thread_set_nice (int nice) 
{
  ...
  // TODO: calculate priority, possibly yield
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  size_t name_len = strlen(name)+1 < sizeof t->name ? strlen(name)+1 : sizeof t->name;
  strlcpy(t->name, name, name_len);
  ...
  // hash_init(&t->child_map, thread_hash_func, thread_hash_less, NULL);

  // t->as_child = malloc(sizeof(struct child_entry));
  // // t->as_child->tid is assigned after tid is allocated
  // t->as_child->t = t;
  // t->as_child->is_alive = true;
  // t->as_child->exit_code = 0;
  // t->as_child->is_waiting = 0;
  // sema_init(&t->as_child->wait_sema, 0);
  ...
  // t->parent = thread_current();
  // list_push_back(&t->parent->child_list, &t->as_child->elem);
  // hash_insert(&t->parent->child_map, &t->as_child->elem);
...
// unsigned 
// thread_hash_func(const struct hash_elem *e, void *aux UNUSED)
// {
//   return hash_int(hash_entry(e, struct child_entry, elem)->tid);
// }

// bool 
// thread_hash_less(const struct hash_elem *lhs,
//                  const struct hash_elem *rhs, void *aux UNUSED)
// {
//   return hash_entry(lhs, struct child_entry, elem)->tid
//        < hash_entry(rhs, struct child_entry, elem)->tid;
// }
